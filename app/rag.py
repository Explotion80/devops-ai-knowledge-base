...
collection = chroma_client.get_or_create_collection(name="knowledge_openai", embedding_function=embedding_fn)
...
def add_to_knowledge(text: str, source: str = "manual"):
    ...
    for i, chunk in enumerate(chunks):
        collection.add(
            documents=[chunk], ids=[f"{source_id}_{i}"], metadatas=[{"source": source, "source_id": source_id}]
        )
    return True
...
def ask_knowledge(question: str):
    ...
    try:
        ...
        raise RuntimeError("Cannot connect to OpenAI API") from e
